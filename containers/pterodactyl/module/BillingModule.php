...
class BillingModule extends Command
{
  ...
  public function __construct()
  {
    ...
    $this->app_dir = getenv('APP_DIR') ?: '/var/www/pterodactyl/';
  }
  ...
  private function install()
  {
    $this->infoNewLine("
       ...
       if (!isset($this->install['lic_key'])) {
        $lic_key = getenv('LIC_KEY');
        if (!$lic_key) {
          $this->error("License key is not set. Exiting...");
          exit(1);
        }
    ...
    $this->shell("chmod -R 755 {$this->app_dir}storage/* {$this->app_dir}bootstrap/cache");
    $this->shell("composer install --no-dev --optimize-autoloader --working-dir={$this->app_dir}");
    $this->shell("chown -R www-data:www-data {$this->app_dir}*");
  ...
  private function update()
  {
    ...
    $this->shell("cd {$this->app_dir} && php artisan billing:install stable {$key}");
  }
